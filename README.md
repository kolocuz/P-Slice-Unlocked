...
**P-Slice Unlocked** is a modified fork of the P-Slice Engine, which itself is a crossover between Psych Engine and the newest version of Friday Night Funkin (V-Slice).
...
lime test <platform>
```
where `<platform>` gets replaced with `windows`, `linux`, or `mac`. You can also add the `-final` flag for release builds.
